            ...
            <div class="card-header ">
             <div class="card-title">
              <h3 class="card-label">
                ...
                <span class="text-dark mx-3">@lang('site.edit') @lang('site.task')</span>

              </h3>
             </div>
            </div>
            <div class="card-body mx-auto col-md-10 ">
                <form method="POST" action="{{ route('dashboard.task.update',$task->id) }}"enctype="multipart/form-data">
                    @csrf
                    @method('Put')
                    <div class="form-group  ">
                        <label class="font-size-h6 font-weight-bolder text-dark">@lang('site.title')</label>
                        <input type="text" name="title" class="form-control @error('title') is-invalid @enderror form-control-solid h-auto p-5 border-0 rounded-lg font-size-h6"  placeholder="@lang('site.title')" value="{{ $task->title }}">
                        @error('title')
                    ...
                    <div class="form-group  ">
                        ...
                        <textarea name="description"class="form-control @error('description') is-invalid @enderror form-control-solid h-auto p-5 border-0 rounded-lg font-size-h6"  placeholder="@lang('site.description')" value="" rows="3" style="margin-top: 0px; margin-bottom: 0px; height: 96px;resize:none">{{ $task->description }}</textarea>
                        @error('description')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                    ...
                    <div class="form-group  ">
                        ...
                        <div class="image-input image-input-outline image-input-circle" id="kt_image_3">
                            <div class="image-input-wrapper" style="background-image: url({{$task->path_image}})"></div>
                    ...
                    <div class="form-group">
                        <label class="font-size-h6 font-weight-bolder text-dark d-block">@lang('site.subject')</label>
                            ...
                            @foreach($subjects as $subject)
                                <option value="{{$subject->id}}"{{$task->subject_id==$subject->id ? 'selected' : ''}}>{{$subject->title}}</option>
                            @endforeach
							</select>
							</div>
                    <div class="form-group">
